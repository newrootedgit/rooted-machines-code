...
def migrate_remove_airknife(path: str):
    """
    One-time migration: prune the obsolete `airknife_mode` key from an existing
    JSON file (the machine no longer has an airknife). Removes it from
    `variable_ranges` and from every per-variety settings dict, then rewrites
    the file only if something actually changed.
    """
    ...
    if isinstance(ranges, dict) and "airknife_mode" in ranges:
        del ranges["airknife_mode"]
        changed = True

    for value in data.values():
        if isinstance(value, dict) and "airknife_mode" in value:
            del value["airknife_mode"]
            changed = True

    if changed:
        locked_atomic_write_json(path, data)
        print("poll: migrated JSON - removed obsolete airknife_mode keys")
...
def main():
    ...
    # Prune obsolete airknife_mode keys from any pre-existing JSON
    migrate_remove_airknife(JSON_FILE_PATH)
