# Create the setup guide text file
content = """Conveyor Seeder Poll + TCP Server Setup Guide (Ubuntu 24.04 LTS, Raspberry Pi 4)

Assumptions
- User: rooted
- Project dir: /home/rooted/te-cli
- Scripts: conveyor_seeder_poll.py, conveyor_seeder_tcp_server.py
...
if not os.path.exists(p):
    with open(p,"w") as f: json.dump({"roller_speed":0,"mode":0}, f, indent=4)
print("ok")
PY
```
...
Poller service:
```bash
sudo tee /etc/systemd/system/conveyor_seeder_poll.service >/dev/null <<'EOF'
[Unit]
Description=Conveyor Seeder Poll
...
TCP server service:
```bash
sudo tee /etc/systemd/system/conveyor_seeder_tcp_server.service >/dev/null <<'EOF'
[Unit]
Description=Conveyor Seeder TCP Server
After=network-online.target
Wants=network-online.target
[Service]
Type=simple
...
sudo systemctl enable conveyor_seeder_poll.service
sudo systemctl enable conveyor_seeder_tcp_server.service
sudo systemctl start conveyor_seeder_poll.service
sudo systemctl start conveyor_seeder_tcp_server.service
```
...
----------------------------------------
```bash
sudo systemctl is-active conveyor_seeder_poll.service
sudo systemctl is-active conveyor_seeder_tcp_server.service
```
...
```bash
ss -ltnp | grep 8888
nc 192.168.10.1 8888   # expect something like: 42,1
```
...
```bash
# Edit conveyor_seeder_tcp_server.py and set HOST = "0.0.0.0"
sudo systemctl restart conveyor_seeder_tcp_server.service
```

2) Poller permission issues / no touch encoders found:
```bash
# Temporary: run poller as root
sudo sed -i 's/^User=rooted/User=root/' /etc/systemd/system/conveyor_seeder_poll.service
sudo systemctl daemon-reload
sudo systemctl restart conveyor_seeder_poll.service

# Proper: fix udev rule (see above), then switch back to User=rooted
sudo sed -i 's/^User=root$/User=rooted/' /etc/systemd/system/conveyor_seeder_poll.service
sudo systemctl daemon-reload
sudo systemctl restart conveyor_seeder_poll.service
```
...
3) Minimal logs:
```bash
sudo journalctl -u conveyor_seeder_poll.service -n 50 --no-pager
sudo journalctl -u conveyor_seeder_tcp_server.service -n 50 --no-pager
```

----------------------------------------
What changed vs the previous guide
----------------------------------------
1) Replaced crontab with systemd for autorun + auto-restart of two scripts (poller + TCP server).
2) Added OS libs and pip package 'hidapi' so 'te' can import HID support.
3) Kept Grayhill udev rules; added optional explicit hidraw rule when needed.
4) Introduced JSON state file shared via proper file locking between two processes.
"""
path = "/mnt/data/conveyor_seeder_setup_guide.txt"
