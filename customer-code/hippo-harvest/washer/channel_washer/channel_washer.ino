...
void setup() {
    ...
    Solenoid.Mode(Connector::OUTPUT_DIGITAL);
...
void loop() {
    ...
    float voltage = ConnectorA10.AnalogVoltage();
    Serial.print(voltage);

    Serial.print(inputState1);
    Serial.print(inputState2);

    // Determine current drive velocity from the input selectors
    int currentVelocityIndex = 3;
    if (!inputState1 && !inputState2) {
        currentVelocityIndex = 3;
    } else if (inputState1) {
        currentVelocityIndex = 2;
    } else if (inputState2) {
        currentVelocityIndex = 4;
    }

    // --- Solenoid: speed-scaled retriggerable off-delay ---
    static unsigned long lastTrayDetectedTime = 0;
    static bool solenoidActive = false;
    unsigned long solenoidOffDelayMs = solenoidOffDelayForVelocity(currentVelocityIndex);

    bool trayDetected = digitalRead(trayDetectionPin);
    if (trayDetected) {
        // Tray present: keep solenoid on and (re)start the timer
        lastTrayDetectedTime = millis();
        Solenoid.State(true);
        solenoidActive = true;
    } else if (solenoidActive) {
        // No tray: hold open until the speed-based delay has elapsed
        if (millis() - lastTrayDetectedTime >= solenoidOffDelayMs) {
            Solenoid.State(false);
            solenoidActive = false;
        }
    }

    // Drive the motor at the selected velocity
    RampToVelocitySelection(currentVelocityIndex);
...
// Returns the solenoid off-delay (ms) for a given drive velocity.
// Delay scales inversely with speed: slower conveyor -> longer hold.
//   velocity 2 = slowest -> 10 s
//   velocity 3 = medium  -> 7.5 s
//   velocity 4 = fastest -> 5 s
unsigned long solenoidOffDelayForVelocity(int velocityIndex) {
    switch (velocityIndex) {
        case 2:  return 10000;  // slowest
        case 3:  return 7500;   // medium
        case 4:  return 5000;   // fastest
        default: return 10000;  // safe default: longest hold
    }
}
