...
#
#   scp install-shutdown-button.sh rooted@100.119.67.117:/tmp/
#   ssh -t rooted@100.119.67.117 'sudo bash /tmp/install-shutdown-button.sh'
#
# `sudo` is NOT passwordless on this Pi, so the second command needs `ssh -t`.
#
...
# is what corrupted BOTH Python scripts on THIS Pi on 2026-08-11: runs of 0xFF
# starting on clean 4 KiB boundaries, both services crash-looping, twenty
# minutes down and an hour of remote debugging. The card itself was never
# replaced, so the exposure is still live — this button is how it stops being
# re-created every evening.
#
# The operator routine this supports:
#
#     press the Shut Down button on screen 2 of the HMI
...
#     poll sees the latch      ->  clears ready_to_run
...
#
# Triggered by rooted-shutdown.path when the Touch Encoder shutdown button
...
note ""
note "To test the real path, press the Shut Down button on HMI screen 2."
