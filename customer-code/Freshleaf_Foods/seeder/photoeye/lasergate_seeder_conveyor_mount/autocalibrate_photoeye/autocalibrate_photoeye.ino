...
// Previous state of the calibrate_request CSV field, for rising-edge detection.
// Starts true so that a request already latched high when we boot (Pi restarted
// mid-request, or we power-cycled while the flag was set) is NOT replayed as a
...
// Control-plane return channel, read by the Pi's TCP server process so it can
// mirror cal_state into the shared JSON for the Touch Encoder poll script.
// Deliberately NOT port 9999: that one is bound by the AWS telemetry ingest,
// and two processes can't share it. Sent every status tick rather than only on
// change, so a single dropped datagram can't strand the TE display.
unsigned int       calStatePort         = 9997;
...
void parseReceivedMessage(char *message) {
    ...
    // Operator-requested recalibration. EDGE triggered (0->1) on purpose: the
    // Pi holds the request high until it sees us leave CAL_DONE, so a level
    // test would re-arm calibration on every heartbeat and the machine would
    // never finish calibrating. The rising edge is the command; the Pi clears
    // the flag once cal_state confirms we acted on it.
    if (calibrate_request_int && !prevCalibrateRequest) {
...
void SendStatusUpdate() {
    ...
    //
    // This deliberately matches the schema-2 SEEDER layout the Pi's AWS ingest
    // (telemetry_ingest.py) and the API already expect, so this machine needs
    // no downstream changes to report into the cloud pipeline.
    //
    // belt_motor_uptime_ms is a STRUCTURAL PLACEHOLDER, always zero. There is
    // no belt motor on this machine — the conveyor is a standalone VFD we do
    // not drive or measure. It is sent as a literal 0 rather than mirroring
    // some other counter on purpose: a plausible-looking fake runtime would
    // feed maintenance scheduling and dashboards with a number nobody can act
    // on. Zero reads as "not present", which is the truth.
    //
    // The calibration diagnostics (cal_state, measured dwell, belt speed,
    // reject count, per-actuator travel times) are NOT in this frame — the
    // schema-2 layout has no slots for them and appending extras would get
    // re-joined into active_variety_name by the ingest's overflow handling.
    // They go out on the CAL_STATE control-plane frame instead; see
    // SendCalState().
    //
    // varietyName is LAST so any snprintf truncation chops the name, not
    // the structured numeric tail.
    const unsigned long beltMotorUptimeMs = 0;  // no belt motor on this machine
...
// SendStatusUpdate because that frame goes to the AWS ingest and is schema
// versioned against it; this one is a private contract with the TCP server and
// deliberately tiny so it can't fragment.
//
// This frame also carries the calibration diagnostics that do not fit the
// schema-2 STATUS_UPDATE layout. They would otherwise exist nowhere, and they
// are exactly what you want when calibration misbehaves on the machine.
//
// Format (schema_ver 1):
//   CAL_STATE,1,bootId,seq,cal_state,belt_speed_x100,cal_block_ms,cal_rejects,
//   travel_irrigation_ms,travel_hopper_ms,travel_misting_ms
//
//   cal_state: 0 = waiting for a tray, 1 = measuring, 2 = done
//   belt_speed_x100: measured belt speed in HUNDREDTHS of an inch/sec — integer
//     so we don't depend on %f support in the toolchain's snprintf. Divide by
//     100 on the Pi side.
//
...
void SendCalState() {
    ...
    snprintf(buf, sizeof(buf), "CAL_STATE,1,%lu,%lu,%d,%lu,%lu,%lu,%lu,%lu,%lu",
             (unsigned long)t.bootId,
             (unsigned long)t.seq,
             (int)calState,
...
void SendEvent(const char *eventCode, const char *motor) {
    ...
    // Already matches the ingest's schema-2 EVENT layout field for field; only
    // the version number needed correcting.
    snprintf(telemetryBuffer, sizeof(telemetryBuffer),
...
void loop() {
  ...
  // surfaces as a warning instead of silently skewing the control loop.
  {
      ...
      if (now - t.lastStatusMs >= STATUS_INTERVAL_MS) {
          ...
          SendStatusUpdate();
          SendCalState();   // control-plane copy for the TE display
