...
// Sensei washer.
//
// Cloned from the Hippo Harvest channel washer, with the tray detection input
// and the solenoid removed — this machine has neither. All it does is read the
// speed selector and drive the motor at the matching velocity.
//
// NOTE THE PIN MOVE. On the Hippo machine the speed selector was on DI6/DI7 and
// IO0 drove the solenoid. Here the selector is wired to IO0/IO1 and nothing
// drives IO0 as an output. Cross-flashing the two sketches would therefore
// energise a solenoid line that is now a selector input, so they are not
// interchangeable even though the motor logic is identical.

#define DriveMotor ConnectorM0

#define inputPin1 IO0  // Drive Speed 1
#define inputPin2 IO1  // Drive Speed 2
...
void setup() {
    ...
    // IO0 and IO1 are bidirectional on the ClearCore; setting them INPUT here
    // is what keeps IO0 from being driven as the output it was on the Hippo
    // machine.
